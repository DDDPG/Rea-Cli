...
| Repository visibility | Private at inspection | Before changing visibility, resolve or exclude each tracked upstream item under its own license/permission. The project MIT scope does not clear `reference/` or `schema/rpp/evidence/`; carry each item's attribution and the `cc-by-nc` noncommercial condition. See [THIRD_PARTY_NOTICES.md](../../THIRD_PARTY_NOTICES.md). Changing visibility remains the maintainer's decision |
