...
| `schema/rpp/evidence`、`reference/` 下的知识语料、逐字段证据与 RPP 前后对照捕获 | 文件清单与来源 manifest 已记录上游路径、来源散列与编辑改动；本次扫描未发现绝对路径、用户名或主机名残留 | `reference/SOURCES.md` 按主题记录来源和署名，但部分来源的许可证/再分发条件仍未确认 | **有条件** | 只有在每项来源条款或单独许可允许时才随公开仓库发布，并保留链接、署名和适用条件；这些文件不进入 wheel、sdist、文档站和 agent bundle，但这是打包边界，不是授权边界 |
...
因此，来源门禁不会依据维护者口径自动清除。发行物必须按来源保留署名、链接、许可证文本和其他条件；没有可核实再分发依据的材料应先排除。若未来改变用途或进入商业分发，应重新检查 `cc-by-nc` 等限制和所有适用许可证。
