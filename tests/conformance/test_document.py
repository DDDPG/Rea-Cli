...
from rac.rpp import parse as rac_parse, patch
...
def test_fx_missing_field_persists_in_owner_and_saved_document(tmp_path):
    doc=parse(SESSION);fx=doc.project.tracks[0].fx_chain.fxs[0];fx.wet=.25
    assert doc.project.tracks[0].fx_chain.fxs[0].wet==.25
    assert doc.project.tracks[0].fx_chain.fxs[1].wet==.8
    out=doc.save(tmp_path/'output.rpp')
    assert parse(out).project.tracks[0].fx_chain.fxs[0].wet==.25
    assert emit(doc).replace('      WET 0.25\n','')==SESSION

def test_take_insert_targets_actual_range(tmp_path):
    doc=parse(SESSION);takes=doc.project.tracks[0].items[0].takes
...
def test_rac_patch_and_object_views_share_one_document():
    assert rac_parse is parse
    doc=parse(SESSION);view=doc.project.tracks[0]
...
def test_endings_bom_and_no_final_newline(ending):
    src='\ufeff'+SESSION.rstrip('\n').replace('\n',ending)
    doc=parse(src);assert doc.text()==src
...
def test_mixed_endings_unchanged_outside_patch():
    ...
    doc=parse(src);doc.project.tracks[0].name='Changed'
    assert doc.text()==src.replace('NAME "Original"','NAME Changed')

def test_invalid_utf8_and_opaque_bytes_preserved(tmp_path):
    src=SESSION.encode().replace(b'AAAAAA==',b'ABC\xff==')
    p=tmp_path/'input.rpp';p.write_bytes(src)
    doc=parse(p);doc.project.tracks[0].name='Changed';doc.save(tmp_path/'out.rpp')
    assert (tmp_path/'out.rpp').read_bytes()==src.replace(b'NAME "Original"',b'NAME Changed')

def test_save_refuses_overwrite(tmp_path):
    doc=parse(SESSION);p=doc.save(tmp_path/'out.rpp')
    with pytest.raises(FileExistsError): doc.save(p)
    doc.save(p,overwrite=True)

def test_repeat_unknown_fields_stay_accessible():
    doc=parse(SESSION);node=doc.tracks()[0].find_chunk('FUTURE')
...
def test_confirmed_field_metadata_and_raw_unknown():
    doc=parse('<REAPER_PROJECT 0.1 7.48 1\nRENDER_RANGE 2 1.25 2.5 0 1000\nNEW_FLAG abc\n>\n')
...
def test_range_deletion_does_not_remove_adjacent_fx():
    doc=parse(SESSION);doc.project.tracks[0].fx_chain.fxs[0].remove()
...
@pytest.mark.parametrize('src',['<REAPER_PROJECT_BAD 1\n>\n','<REAPER_PROJECT 1\n<TRACK\n'])
def test_invalid_structure_errors(src):
    with pytest.raises(RPPParseError): parse(src)
