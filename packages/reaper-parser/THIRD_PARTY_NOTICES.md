...
- `src/rac/data/knowledge/rpp_schema.json` was generated by the original project
  from **DDDPG/ReaperDoc**, `constants.ts` at commit `32047bb`:
  [DDDPG/ReaperDoc](https://github.com/DDDPG/ReaperDoc). The recorded source does not contain a
  LICENSE, and package.json supplies no license. Descriptions and metadata retain
  their upstream provenance. Public availability is not a redistribution license.
...
- The annotated RPP tree and extraction notes derive from ReaperDoc at the
  commit above, with the same unresolved source-license status.
