...
See the [three-harness quick start](docs/harness/README.md) and the
...
**Status:** `0.1.0` alpha. Install from PyPI for the published packages, or from
a source checkout for the current repository code. A checkout may be newer than
the last PyPI upload. The API may still change. The distribution and CLI are
named `reacli`; Python imports use `rac`.
...
The repository keeps runtime code, examples and Git-only reference material separate:
...
- [REAPER agent skill](integrations/agents/reaper-agent-cli/SKILL.md) · [Repository skill usage](integrations/agents/README.md) · [中文使用说明](integrations/agents/reaper-agent-cli/GUIDE.zh-CN.md)
...
[reference/SOURCES.md](reference/SOURCES.md) for the per-source record.
