...
`evidence/live-shapes/`, with [live-validation.json](evidence/live-validation.json).
These are audit records: they document what was verified on a real host and are
not executable fixtures.
...
repository clones it too. It is not covered by the project MIT license; the current
repository delivery retains each source's terms and attribution. See the
